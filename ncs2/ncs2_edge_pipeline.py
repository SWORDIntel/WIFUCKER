...
class BatchProcessor:
    ...
    def __init__(
        self, min_batch_size: int = 1, max_batch_size: int = 32, target_latency_ms: float = 5.0
    ):
    ...
    def should_process_batch(self, queue_size: int, time_since_last_batch: float) -> bool:
        """
    ...
    def update_batch_size(self, latency_ms: float, throughput_ops_per_sec: float):
        """
...
class DevicePipeline:
    ...
    def __init__(self, device_id: int, memory_pool: NCS2MemoryPool, max_parallel_graphs: int = 4):
        """
        ...
        self.batch_processor = BatchProcessor(
            min_batch_size=1, max_batch_size=32, target_latency_ms=2.0  # Aggressive 2ms target
        )
    ...
    def start(self):
        ...
        self.workers = [
            ...
            threading.Thread(
                target=self._inference_worker, name=f"Device{self.device_id}-Inference", daemon=True
            ),
            threading.Thread(
        ...
        while self.running:
            try:
                ...
                if self.batch_processor.should_process_batch(
                    len(batch_buffer), time.time() - last_batch_time
                ):
                    if batch_buffer:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "min_latency_ms": (
                self.stats.min_latency_ms if self.stats.min_latency_ms != float("inf") else 0.0
            ),
...
class NCS2EdgePipeline:
    ...
    def __init__(self, device_count: int = 3, max_parallel_graphs_per_device: int = 4):
        """
        ...
        self.memory_manager = get_memory_pool_manager(
            device_count=self.device_count, pool_size_mb=512
        )
    ...
    def get_total_tops(self) -> float:
        ...
        return sum(pipeline.stats.achieved_tops for pipeline in self.pipelines.values())

    def get_stats(self) -> Dict:
        """Get statistics for all devices."""
        device_stats = {
            device_id: pipeline.get_stats() for device_id, pipeline in self.pipelines.items()
        }

        total_throughput = sum(stats["throughput_ops_per_sec"] for stats in device_stats.values())
        ...
        return {
            ...
            "performance_ratio": (
                total_tops / (self.device_count * 10) if self.device_count > 0 else 0.0
            ),
...
def get_edge_pipeline(device_count: int = 3, max_parallel_graphs: int = 4) -> NCS2EdgePipeline:
    """
    ...
    if _edge_pipeline is None:
        _edge_pipeline = NCS2EdgePipeline(
            device_count=device_count, max_parallel_graphs_per_device=max_parallel_graphs
        )
