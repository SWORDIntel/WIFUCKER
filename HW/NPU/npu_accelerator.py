...
class IntelNPUAccelerator:
    ...
    def _detect_military_npu(self) -> Optional[NPUCapabilities]:
        """Detect secondary military NPU if available."""
        # Check for military NPU at alternative PCI address or via DSMIL device
        military_pci_paths = [
            "/sys/devices/pci0000:00/0000:00:0c.0",  # Alternative PCI slot
            "/sys/devices/pci0000:00/0000:00:0d.0",  # Another possible slot
        ]
        
        # Check for military mode indicator
        military_mode_file = Path("/opt/lat5150/config/military_mode")
        dsmil_military_env = os.environ.get("LAT5150_MILITARY_MODE") == "1"
        
        is_military = military_mode_file.exists() or dsmil_military_env
        
        if is_military:
            # Check for secondary NPU device
            for pci_path in military_pci_paths:
                if Path(pci_path).exists():
                    logger.info(f"Detected secondary military NPU at {pci_path}")
                    return NPUCapabilities(
                        model_name="Intel Core Ultra 7 165H (Military NPU)",
                        architecture="Meteor Lake-P (Military Mode)",
                        pci_id=pci_path.split("/")[-1],
                        pci_device="8086:7e00",  # Same device ID, different instance
                        revision="04",
                        tops_int8=34.0,  # Military mode: 34-49.4 TOPS
                        tops_int4=68.0,
                        tops_fp16=17.0,
                        tops_optimized=49.4,  # Maximum military mode performance
                        tiles=2,
                        streams_per_tile=2,
                        memory_mb=128,  # Military NPU has larger cache
                        max_power_w=8.0,  # Higher power for military mode
                        supports_int4=True,
                        supports_int8=True,
                        supports_fp16=True,
                        supports_openvino=True
